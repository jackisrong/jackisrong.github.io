// Use selected session to populate course names
...
// Use chosen course name to populate course numbers
...
function chosenCourseSubmit() {
	...
	if (selectedSession == "" || selectedCourseName == "" || selectedCourseNumber == "") {
		...
		window.open('selected_course.html?session=' + selectedSession + '&courseName=' + selectedCourseName + '&courseNumber=' + selectedCourseNumber, '_blank');
	}
}
...
function getSectionInfo(session, courseName, courseNumber, section) {
	...
	request.onload = function () {
		...
		if (request.status >= 200 && request.status < 400) {
			// Create array with relevant section data and store into invisible HTML element
			// This is a workaround of the async nature of XMLHttpRequests and allows sections to be displayed in order of section code
			...
			console.log('We have encountered an error when accessing the UBC Courses API when fetching the section information.')
		}
	}
...
// Get temporary data stored in invisible HTML element and display results
function getTempData() {
	// Evaluate temp data that was previously stored into invisible HTML element
	...
	// This is inefficient because it repopulates the table after every call (a course with 8 sections would repopulate it 8 times)
	// This is due to the async nature of XMLHttpRequests and the workaround that we're using
	// However, the efficiency is negligible because most modern systems will be able to handle it within seconds
	for (var i = 0; i < tempDataArray.length; i++) {
		...
		// Manipulate time and date data in anticipation of timetablePreview population
...
// Create the timetable preview (using a JavaScript function rather than hard coding HTML to save my sanity and copy-paste fingers)
...
// Show/hide timetable button functionality
function toggleTimetable() {
	if (document.getElementById("timetablePreview").style.display == "block") {
		document.getElementById("timetablePreview").style.display = "none";
	} else {
		document.getElementById("timetablePreview").style.display = "block";
	}
}
